...
@section('title', Lang::get('admin.editattributegroup'))
@section('headtitle', Lang::get('admin.editattributegroup'))

@section('toolbar')
{!! Form::open(['url' => '/admin/catalog/attributegroup/update/' . $group->id_attribute_group]) !!}
...
<div class="row">
	...
	<div class="col-md-9 col-xs-12">		
		<div class="tab-content" id="adminTabContent">      		
      		<div class="tab-pane fade show active" id="panel-data" role="tabpanel" aria-labelledby="tab-data">
				...
				<div class="tab-content" id="langTabContent">
				  @foreach(array_keys(Config::get('languages')) as $key)
				  @php 
				  $groupDescription = $group->Descriptions($key);
				  @endphp
				  <div class="tab-pane fade {{ $firstPanel }}" id="panel-{{$key}}" role="tabpanel" aria-labelledby="tab-{{$key}}">
					...
					<div class="input-group mb-2">
						...
						{!! Form::text('name-' . $key, old('name-' . $key ,$groupDescription->name), ['class' =>'form-control' . ($errors->has('name-' . $key) ? ' border-danger' : '')]) !!}
					</div>
					@php 					
		...
		<div class="input-group mb-2">
			...
			{!! Form::number('sort_order', old('sort_order', $group->sort_order), ['class' =>'form-control' . ($errors->has('sort_order') ? ' border-danger' : '')])  !!}
		</div>
		@php 					
